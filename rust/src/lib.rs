...
pub(crate) fn unwrap_exc_or<T>(exc: Result<T, ()>, def: T) -> T {
    match exc {
        Ok(value) => value,
        Err(_) => def,
    }
...
pub(crate) fn unwrap_exc_or_null<T>(exc: Result<T, ()>) -> T
where
...
pub(crate) fn ptr_from_vec<T>(v: Vec<T>) -> (*mut T, usize) {
...
pub(crate) fn free_ptr_from_vec<T>(ptr: *mut T, len: usize) {
