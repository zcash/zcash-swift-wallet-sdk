...
**Nested-framework landmine (FIXED 2026-06-11 in init-local-ffi.sh, kept for archaeology):** the script's install step used `cp -R built.xcframework LocalPackages/libzcashlc.xcframework` — when the destination already existed, `cp -R` copied the new framework INSIDE it (`libzcashlc.xcframework/libzcashlc.xcframework/...`), leaving STALE slices at the top level that SPM/Xcode silently use → device runs old code while the fresh build sits one directory deeper. The script now does `rm -rf` before installing (both source-build and `--cached` paths). If you ever see a nested `libzcashlc.xcframework` inside `LocalPackages/libzcashlc.xcframework/`, the layout is corrupt: `rm -rf LocalPackages && ./Scripts/init-local-ffi.sh`. Freshness check that catches stale binaries when headers look fine: `strings <slice>/libzcashlc.framework/libzcashlc | grep -c '<a log string unique to your new code>'`.
...
- Modules in core: `grpc`, `chunk`, `fetch`, `verify`, `block_source`, `wallet_session`, `scan`, `scheduler`, `enhance`, `transparent`, `events`, `engine`, `oracle`, `persist`, `mempool`.
...
**Definitive freshness check (added 2026-06-12 after a stale-device-framework field run):** `slipstream_core::engine::ENGINE_BUILD` is logged at every sync start (`engine pass starting engine_build=<tag>`) and in the stage-split line. BUMP the tag on every perf-relevant engine change. A device log whose tag doesn't match HEAD's value = stale framework on the device — no other diagnosis needed. Probe a built slice: `strings <slice>/libzcashlc.framework/libzcashlc | grep <tag>`.
