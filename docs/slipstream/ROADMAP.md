...
| P1 | Transport | Parallel `GetBlockRange` fetchers → ordered in-memory chunks; continuity verify; fetch benchmark CLI | **G1:** hermetic darkside fetch test green; on public lwd, parallel streams ≥2× single-stream MB/s (recorded in STATE.md). MEASURED 2026-06-10: criterion is the ratio, not a specific K — K=4 hit 2.56× once; K=8 hit ≥2× in 3/5 runs (max 3.86×); heavy server-side throttling variance. K=8 is the better default for zec.rocks (input for P5 adaptivity). |
| P2 | Scan core | `MemBlockSource`; persistent WalletDb session (open once, WAL); chunked `scan_cached_blocks` (≈10k); scheduler v0 (verify→tip-window→linear backfill via existing scan-queue); fetch∥scan pipeline | **G2:** darkside wallet syncs, found-tx matches fixture; 1M-block mainnet range completes via CLI; ≥5× old-SDK wall-clock on same machine/server (recorded) MEASURED 2026-06-10: slipstream CLI absolutes in STATE.md (50k=32.4 s, 1M=317.2 s). RATIO MEASURED 2026-06-11 (user device, real wallet, 268855 blocks): 158s vs 1710s = 10.8× — ≥5× criterion EXCEEDED. |
| P3 | Completeness | Rust enhancer (concurrent GetTransaction → decrypt_and_store), transparent/UTXO refresh, event+snapshot surface | **G3 (engine-complete):** differential parity script passes — balances + tx history in `data.db` identical to an old-SDK-synced db for darkside fixtures and ≥1 real mainnet test seed MEASURED 2026-06-11: darkside-oracle parity confirmed (enhanced fields: raw IS NOT NULL for both fixtures, txs_stored==2, balance==200000 zatoshi; darkside serial 6/6); full-path mainnet 50k: 30.4s / 50012 blk / fetch 1.1s / scan 30.0s, bound=scan; mainnet-seed differential parity needs-user (Blockers). |
...
| P6 | Sparse tree persistence (T6.1–T6.7 DONE; T6.8 quick levers next) | Golden-oracle harness; checkpoint-downgrade spike (T6.3b GO: 5.05×); oracle-clean (T6.4: hermetic+darkside+mainnet IDENTICAL); reorg+truncate (T6.5: darkside 9/9 green); default-ON (T6.6: Mac 5.27×, OfflineTests 463/0, XCFramework rebuilt) | **G6 device MEASURED 2026-06-12 (269k restore):** iPad A10 **9:21** (was 16:50; old SDK 1:04:26 = 6.9×); iPhone 16 Pro **1:35** (was 3:28). **Zingo beaten both devices** (11:34 / 2:41). Goal lines NOT yet crossed: engine 554s vs <533s, 90s vs <73s (Zkool 8:53 / 1:13). T6.8 L1+L2 projected to cross; L4 decrypt-kernel = the beat-Zkool lever. Mac: 5.27×. |
