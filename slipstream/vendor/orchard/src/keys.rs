...
impl PreparedIncomingViewingKey {
    ...
    /// [slipstream fork] raw ivk scalar for the batched same-scalar DH kernel.
...
impl PreparedEphemeralPublicKey {
    ...
    /// [slipstream fork] raw ephemeral point for the batched same-scalar DH kernel.
    pub(crate) fn raw_point(&self) -> pallas::Point {
        self.0.raw_point()
    }

    /// [slipstream fork] test-only: wrap a raw point as a prepared epk (the
    /// batch_dh KAT fabricates ephemeral keys directly on the curve).
    #[cfg(test)]
    pub(crate) fn testing_from_point(p: pallas::Point) -> Self {
        PreparedEphemeralPublicKey(PreparedNonIdentityBase::new(
            crate::spec::NonIdentityPallasPoint::from_kernel(p),
        ))
    }
}
...
impl SharedSecret {
    /// [slipstream fork] Wraps a `[ivk]·epk` product the batched same-scalar
    /// DH kernel produced (byte-identical to `agree` — KAT-gated in
    /// batch_dh.rs; the kernel poison-marks identity results, so `p` is
    /// non-identity by construction).
    pub(crate) fn from_kernel_point(p: pallas::Point) -> Self {
        SharedSecret(NonIdentityPallasPoint::from_kernel(p))
    }
