...
impl NonIdentityPallasPoint {
    /// [slipstream fork] Wraps a point the batched same-scalar DH kernel
    /// produced. The kernel poison-marks identity results (affine coordinates
    /// cannot represent them), so every point reaching this is non-identity.
    pub(crate) fn from_kernel(p: pallas::Point) -> Self {
...
#[derive(Clone, Debug)]
pub(crate) struct PreparedNonIdentityBase(
    WnafBase<pallas::Point, PREPARED_WINDOW_SIZE>,
    // [slipstream fork] the raw point, retained for the batched same-scalar
    // DH kernel (batch_dh.rs) — the wNAF table cannot give it back.
    pallas::Point,
);

impl PreparedNonIdentityBase {
    pub(crate) fn new(base: NonIdentityPallasPoint) -> Self {
        PreparedNonIdentityBase(WnafBase::new(base.0), base.0)
    }

    /// [slipstream fork] raw point for the batched same-scalar DH kernel.
    pub(crate) fn raw_point(&self) -> pallas::Point {
        self.1
    }
}
...
pub(crate) struct PreparedNonZeroScalar(
    ...
    // [slipstream fork] the raw scalar, retained for the batched same-scalar
    // DH kernel (batch_dh.rs).
    pallas::Scalar,
);
...
impl PreparedNonZeroScalar {
    ...
    /// [slipstream fork] raw scalar for the batched same-scalar DH kernel.
