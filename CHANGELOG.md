...
- The blocking `TorClient` FFI methods (`prepare()`, `close()`, `isolatedClient()`, `setDormant(mode:)`, `sleep()`, `wake()`, `httpRequest(for:retryLimit:)`, `getExchangeRateUSD()`, `connectToLightwalletd(endpoint:)`) are now `async`. They previously blocked the calling thread for the full duration of the underlying Tor network operation; they now hop onto a private serial queue so that blocking never lands on Swift's cooperative thread pool. Call sites that already `await` these cross-actor calls are unaffected.

## Fixed
- Fixed a crash (`EXC_BREAKPOINT`) and a related deadlock when fetching the ZEC-USD exchange rate over Tor for the first time (MOB-1389). `refreshExchangeRateUSD()` is now single-flight, so concurrent invocations against a not-yet-bootstrapped Tor client no longer race on the synchronizer's shared `exchangeRateTor` state and trap. The lazy bootstrap of the HTTP-over-Tor client (`httpRequestOverTor(for:retryLimit:)`) is single-flighted the same way. Separately, because the blocking Tor FFI calls now run off the cooperative thread pool (see above) and runtime bootstrap is bounded by a caller-side timeout, a slow or stalled Tor bootstrap can no longer hang Swift concurrency.
