...
- `zcash_voting` dependency foundation: SDK Rust crate now depends on `zcash_voting 0.5.5` (`default-features = false`, `client-pir`, `client-tree-sync`) and exposes pure-function FFI symbols for share-nullifier computation and PIR proof validation. No Swift API surface is added in this step.
...
- Bumped `zcash_voting` to `0.5.5` so `network_id` matches the SDK (`0` = testnet, `1` = mainnet) end-to-end for wallet-notes JSON consumed by delegation PIR, delegation submission signing respects non-zero account indexes, and Keystone submissions validate the signed sighash against the stored PCZT sighash. No public Swift API changes.
