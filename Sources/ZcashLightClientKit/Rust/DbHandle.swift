...
final class WalletDbHandle: Sendable {
    ...
    private nonisolated let dbDataPath: URL

    /// The network type (mainnet or testnet).
    private nonisolated let networkType: NetworkType

    /// Returns the URL path to the wallet database file.
...
final class FsBlockDbHandle: Sendable {
    ...
    private nonisolated let fsBlockDbRootPath: URL

    /// Returns the URL path to the block cache database root directory.
    nonisolated var databaseURL: URL {
        fsBlockDbRootPath
    }
