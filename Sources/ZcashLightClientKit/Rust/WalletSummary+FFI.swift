...
//  ZcashLightClientKit
//
//  Created for Slipstream task [#1755] (Engine API v2.1, Phase 1 / D-1).
//
//  The C `FfiWalletSummary` → Swift `WalletSummary` mapping, extracted from
//  `ZcashRustBackend.getWalletSummary()` so the legacy per-DB call and the unified
//  slipstream summary (`zcashlc_slipstream_wallet_summary`) share ONE definition.
//
...
extension WalletSummary {
    ...
    /// `ZcashRustBackend.getWalletSummary()`; shared by the legacy path and the slipstream
    /// unified-summary path (Engine API v2.1 — E-2 later sinks this into the FFI itself).
